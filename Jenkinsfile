         ...
         stage ('Deploy'){
            steps{
                 withCredentials([usernamePassword(credentialsId: 'akash-tom', passwordVariable: 'pass', usernameVariable: 'userId')]) 
                {
                    ...
                    sh 'curl -u  $userId:$pass --upload-file target/springapp-${BUILD_NUMBER}.war http://ec2-35-154-254-182.ap-south-1.compute.amazonaws.com:8080/manager/text/deploy?config=file:/var/lib/tomcat8/springapp-${BUILD_NUMBER}.war\\&path=/Ashish_DEMO'
            }
        }
         }
       /*stage ('Notify'){
